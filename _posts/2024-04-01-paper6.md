...
=======
$$\dot{\Phi}=\Omega+\mathbf{BS} \tag{3.3}$$

where

$$\Phi=\begin{bmatrix}\phi_1 \\ \vdots \\ \phi_{N-1}\end{bmatrix},\ \ \ \mathbf{S}=\begin{bmatrix}\sin\phi_1 \\ \vdots \\ \sin\phi_{N-1}\end{bmatrix}, \ \ \ \Omega=\begin{bmatrix}\Omega_1 \\ \vdots \\ \Omega_{N-1}\end{bmatrix}\tag{3.4}$$
>>>>>>> 51b21e43baf2ddc06b3bd9ea8de73f39e40211d0
...
In particular, from [fig3.5](#fig3.5), the model exhibits, for all $\Omega$, stable forward running phase locked solutions when the long distance coulpling is inhibitory and sufficiently strong for a given excitatory nearest neighbor coupling ($-\beta\gg\alpha\gt0$).
