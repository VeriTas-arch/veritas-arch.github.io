...
<p>
If the net coupling $k$ is weak, $|k|\lt|\Omega|$, e.g. due to a lesion, then no phase locked solutions occur. Rather each of the two oscillators "drifts" in a manner which can be investigated by solving $\eqref{eq:3.20}$, $\eqref{eq:3.20}$ exactly.
</p>
