...
<p>
<b>Assumption</b> (weak coupling). We assume that the coupling is relatively weak, so that $|\alpha|$ is small and $|\mathbf{g}|\ll|\mathbf{f}|$. In such case the limit cycles of the uncoupled oscillators are each slightly perturbed and one can continue touse the phase coordinates $\theta _j$, and amplitude $\mathbf{r}_j$ to characterize the state of each oscillator. In this way one obtains sets of equations of the form
</p>
